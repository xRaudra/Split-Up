...
export default function SettlementRow({ from, to, amount, status, onMarkPaid }) {
  ...
  return (
    <div className="flex items-center gap-2 px-4 py-3.5 rounded-[10px] bg-white" style={{ border: '1px solid #E5E7EB' }}>
      ...
      <span className="font-semibold text-sm text-[#111827]">{from}</span>
      <span className="text-[#9CA3AF] text-sm">→</span>
      <Avatar name={to} size={32} />
      <span className="flex-1 font-semibold text-sm text-[#111827] truncate">{to}</span>
